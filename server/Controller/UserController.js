...
const deleteALikeMovie = asyncHandler(async (req, res) => {
  try {
    const user = await User.findById(req.user._id);
    const movie = await Movie.findById(req.params.id);
    if (user && movie) {
      user.likedMovies = user.likedMovies.filter(likedMovieId => likedMovieId.toString() !== movie._id.toString());
      await user.save();
      res.status(200).json({ success: true });
    } else {
      res.status(404).json({ success: false, message: 'User or movie not found' });
    }
  } catch (error) {
