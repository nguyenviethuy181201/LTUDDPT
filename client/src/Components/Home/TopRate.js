    ...
    >
      {movies.map((movie, index) => (
        <SwiperSlide key={index}>
          <div className="p-4 h-rate group border bg-dry rounded-lg overflow-hidden">
            <img
              src={movie?.titleImage ? movie.titleImage : 'images/user.png'}
              alt={movie?.name}
              className="w-full h-full object-cover rounded-lg"
            />
            <div className="px-4 flex flex-col justify-center items-center hidden group-hover:flex gap-6 text-center absolute bg-black bg-opacity-70 top-0 left-0 right-0 bottom-0">
              <button
                onClick={() => LikeMovie(movie, dispatch, userInfo)}
                disabled={isLiked(movie) || isLoading}
                className={`w-12 h-12 flex-colo transitions hover:bg-subMain rounded-full
                ${isLiked(movie) ? 'bg0subMain' : 'bg-white bg-opacity-30'}
                 text-white`}
              >
                <FaHeart />
              </button>
...
const TopRate = ({ movies, isLoading }) => {
