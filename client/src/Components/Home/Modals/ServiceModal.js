    ...
    return (
        <MainModal modalOpen={modalOpen} setModalOpen={setModalOpen}>
            <div className="flex flex-col gap-[10px]  justify-center items-center  border border-border  w-full  align-middle p-10 bg-dry overflow-y-auto  text-white rounded-2xl ">
                <p className='text-2xl font-bold'>Unlimited Discovery</p>
                        ...
                        services?.map((service, index) => (
                            ...
                            <div key={index} className={  `w-[50%] border rounded flex flex-col justify-center items-center p-5 ${index === 0 ? "bg-gradient-to-tr from-[#a32cdf] to-[#1fc9fd]" : index === 1 ? "bg-gradient-to-tr from-[#ff0a6c] to-[#4a3cdb]" : null }`}>
                                <div className='flex gap-[10px] flex-col justify-center items-center'>
                                    <div>
                                        <span className='text-5xl font-bold'>{service.servicePackage}</span> <span className='text-xl font-semibold'>Days</span>
                                    </div>
                                    <p className='text-center text-lg'> Unlimited Free Movie Streaming Experience</p>
                                    <div className='mt-[20px]'>
                                        <span className='text-4xl font-bold'>{service.price/1000}K</span> <span className='text-xl'>/</span> <span className='text-sm'>{service.servicePackage} Days</span>
                                    </div>
                                    
                                </div>
                                <button onClick={() => {setModalOpen(false);{dispatch(memberRegister({amount: service.price, info : filmCode + '_' + user + '_' + service.servicePackage})) }}} className='mt-[30px] border rounded-3xl py-2 px-6 text-lg   transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-110  duration-300'>
                                    Register Now
                                </button>
                            </div>
                        ))
